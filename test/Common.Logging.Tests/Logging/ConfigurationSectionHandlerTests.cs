...
namespace Common.Logging
{
				...
				public void NoParentSectionsAllowed()
				{
					IConfigurationSectionHandler handler = new ConfigurationSectionHandler();
						Assert.Throws(Is.TypeOf<ConfigurationException>().And.Message.EqualTo("parent configuration sections are not allowed")
												 , delegate {
													 handler.Create(new LogSetting(new List<LogSetting.Entry> { new LogSetting.Entry(typeof(ConsoleOutLoggerFactoryAdapter), null) }), 
																								 null,
																								 null);
													});
				}
		...
		<logging>
			<factoryAdapter type='CONSOLE'/>
						...
						Assert.AreEqual(typeof(ConsoleOutLoggerFactoryAdapter), setting.Entries[0].FactoryAdapterType);
		...
		<logging>
			<factoryAdapter type='TRACE'/>
						...
						LogSetting setting = reader.GetSection(null) as LogSetting;
						Assert.IsNotNull(setting);
						Assert.AreEqual(typeof(TraceLoggerFactoryAdapter), setting.Entries[0].FactoryAdapterType);
		...
		<logging>
			<factoryAdapter type='NOOP'/>
						...
						LogSetting setting = reader.GetSection(null) as LogSetting;
						Assert.IsNotNull(setting);
						Assert.AreEqual(typeof(NoOpLoggerFactoryAdapter), setting.Entries[0].FactoryAdapterType);
		...
		<logging>
			<factoryAdapter type='CONSOLE'>
				...
				<arg key='level' value='DEBUG' />
						...
						LogSetting setting = reader.GetSection( null ) as LogSetting;
						Assert.IsNotNull( setting );
						Assert.AreEqual(1, setting.Entries[0].Properties.Count);
						Assert.AreEqual(3, setting.Entries[0].Properties.GetValues("LeVeL").Length);
				}
		}
}
